...
- Implement a “New Bookmark” form with input fields for Category, URL, Username, and Password.
- Show field-level error messages next to invalid inputs.
- Clear button resets all form fields.
- On successful submission, display the bookmark below in a Grid View (card format).
- Each bookmark card shows Name, Category, URL, Username, and Password.
- Generate the Website Name automatically from the entered URL.
- Create an Icon for each card using the first two letters of the domain and the selected color.
- The Password field is hidden (dotted) by default, and can be toggled with a Reveal button.
- Add a Search feature to filter bookmarks by Name or URL dynamically.
